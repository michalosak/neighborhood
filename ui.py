...
def print_dictionary(table, title_list):
    """
    nice printing dictionary
    """
    ...
    for key, item in table.items():
        try:
            if cell_size[i] < len(str(key)):
                cell_size[i] = len(str(key))
        except:
            cell_size.append(len(str(key)))
    i += 1
    ...
    for key, item in table.items():
        ...
        print('| {:{width}} |'.format(key, width=cell_size[0]), end="")
        print(' {:{width}} |'.format(item, width=cell_size[1]), end="")
...
def print_table(table, title_list):
    ...
    # checking largest cells
    table_size = count_table_size(table, title_list)
